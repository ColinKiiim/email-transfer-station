...
- fix: |AdminNext| 新版后台收件流详情改为选中邮件后解析原始 MIME 并优先隔离渲染 HTML 正文，避免 OpenAI 等格式化邮件退化成轻量纯文本预览
- fix: |Admin API| 修复新版后台收件流轻量解析未解码 quoted-printable/base64 正文的问题，避免邮件详情和列表预览显示 `=20`、`=E2=80=99` 等传输编码片段
- fix: |AdminNext| 修复收件流筛选列拖到最小宽度后继续拖动会错误放大邮件列表的问题，并让域名筛选按真实收件地址域归属，不再把转发原始域误当作列表所属域
- fix: |AdminNext| 收件流去除功能区可见说明文字，改为问号悬浮说明，并修复未读圆点压住邮件标题的问题
...
- improve: |AdminNext| `mail-admin` 收件流删除从灰度 no-op 改为真实生产后端接入：详情删除会删除当前邮件，批量删除会删除当前筛选结果，并在执行前显示生产数据确认
- improve: |AdminNext| 收件流邮件列表改为 Gmail 式已读/未读视觉：已读行更灰且字重更轻，未读行白底且标题更重；侧栏折叠按钮改为品牌左侧三横线，顶部品牌行不参与收缩动画，只让下方导航收紧为图标列
- improve: |AdminNext| 收件流三列间距收紧为 Gmail 式窄分隔，减少筛选、列表和详情之间的空白
