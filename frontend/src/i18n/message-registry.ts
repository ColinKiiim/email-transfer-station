export const MESSAGE_REGISTRY = {
  ...
  "components.AddressCredentialModal": {
    ...
    "recipientShareTip": {
      "en": "Copy a ready-to-send message for the recipient. Warning: The copied link contains the current mailbox credential and grants full access. A revocable read-only temporary share link must be generated by an administrator in the admin console.",
      "zh": "复制后可直接发送给接收人。注意：所复制链接包含当前邮箱凭证并授予完整访问权限；如需可撤销的只读临时分享链接，须由管理员在管理后台生成。"
    },
    "security": {
      "en": "Security",
      "zh": "安全"
  ...
  "views.common.Login": {
    "anonymousDisabledNotice": {
      "en": "Anonymous address creation is unavailable. Please sign in with an existing account or bind addresses in the user center.",
      "zh": "当前系统未开放匿名创建地址，请使用现有账户登录或在用户中心绑定"
    },
    "autoGeneratedName": {
...
export const getMessageSource = <N extends MessageNamespace>(
  ...
  const localized = MESSAGE_REGISTRY[namespace][key] as Record<'en' | 'zh', string>
