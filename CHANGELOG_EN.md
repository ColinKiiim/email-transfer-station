...
- fix: |AdminNext| Parse the selected mail's raw MIME in the new admin mail-flow detail pane and prefer isolated HTML rendering, so formatted mail from senders like OpenAI no longer degrades to the lightweight plain-text preview
...
- fix: |E2E| Keep test helper endpoints gated by `E2E_TEST_MODE` and add optional `E2E_TEST_SECRET` header validation so an accidentally enabled test mode is not directly callable
...
- improve: |Email Worker| Inbound mail for Cloudflare Email Routing direct-receive domains is now stored in the site without triggering global/rule forwarding by default, so moving the 20030405.xyz catch-all to this Worker no longer keeps forwarding those messages to Gmail
- fix: |Admin API| Use lightweight header and preview parsing for the admin mail-flow list so a single complex MIME message cannot make `/api/admin/mails` sync fail with HTTP 500
